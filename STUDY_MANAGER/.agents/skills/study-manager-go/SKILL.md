...
*   **Responsabilidades**:
    ...
    5.  Gerenciar sessões de estudo em tópicos do Telegram (`src/api/main/commands/topic`).

### 🤖 Integração com Gemini CLI (Tutor de Estudos)
*   **Módulo**: `src/api/main/commands/topic`
*   **Papel**: Orquestração de chat assistido por IA dentro de fóruns do Telegram.
*   **Fluxo de Operação**:
    1.  **Abertura**: O comando `HandleStartTopic` cria um novo tópico (`createForumTopic`) e inicializa a sessão no banco.
    2.  **Mensagens**: O webhook roteia mensagens do tópico para `HandleTopicMessage`.
    3.  **Contexto**: O sistema lê a nota física, anexa o histórico de mensagens salvo no CloverDB e aplica o `system_prompt.txt`.
    4.  **IA**: Invoca o binário `gemini` via CLI (`gemini ask`).
    5.  **UX**: Implementa animação de `typing` (Chat Action) e substituição dinâmica de mensagem de loading para uma experiência fluida.
...
*   **Coleção `study_sessions`**:
    *   Gerencia o estado de conversas ativas com o Gemini.
    *   Campos chave: `chat_id`, `thread_id` (ID do tópico), `short_id` (referência à nota) e `history` (array de mensagens Usuário/Tutor).
    *   *Nota Técnica*: Devido ao parse de JSON do CloverDB, IDs numéricos carregados do disco podem ser tratados como `float64`. As funções de busca (`getActiveSession`) implementam fallback de tipo para garantir a continuidade da sessão após reinícios.
...
- **Topic Logic (Gemini)**: `src/api/main/commands/topic/main.go`
- **Topic Prompt**: `src/api/main/commands/topic/system_prompt.txt`

...
1.  **Scanner** (`src/processes/scanner`): Varre o vault Obsidian.
...
2.  **Processor** (`src/processes/processor`): Calcula o status das revisões.
    - Gera IDs únicos (MD5 truncado).
    - Calcula `dias_atraso` e define status (`ATRASADA`, `HOJE`, `EM_DIA`).
3.  **Exporter** (`src/processes/exporter`): Gerencia o estado e formata a saída.
...
4.  **Notifier** (`src/processes/notifier`): Despachante final.
    - Lê o `.env` (Telegram Token/Chat ID).
    - Envia a mensagem formatada via HTTP (API do Telegram).

## 🛠️ Comandos Essenciais

- **Execução Total**: `go run ./src/pipelines/notifySummary` (Compila e roda tudo).
- **Compilação Manual**: `go build -o bin/ ./src/processes/...`
